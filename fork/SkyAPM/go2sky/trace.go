...
import (
	...
	"github.com/767829413/normal-frame/fork/SkyAPM/go2sky/internal/idgen"
...
func (t *Tracer) CreateEntrySpan(ctx context.Context, operationName string, extractor propagation.Extractor) (s Span, nCtx context.Context, traceID string, err error) {
	...
	s, nCtx, traceID, err = t.CreateLocalSpan(ctx, WithContext(refSc), WithSpanType(SpanTypeEntry), WithOperationName(operationName))
	if err != nil {
		return
	}
	return
...
func (t *Tracer) CreateLocalSpan(ctx context.Context, opts ...SpanOption) (s Span, c context.Context, traceID string, err error) {
	if ctx == nil {
		return nil, nil, "", errParameter
	}
	...
	s, traceID, err = newSegmentSpan(ds, parentSpan) //nolint:typecheck
	if err != nil {
		return nil, nil, "", err
	}
	return s, context.WithValue(ctx, ctxKeyInstance, s), traceID, nil
}
...
func (t *Tracer) CreateExitSpan(ctx context.Context, operationName string, peer string, injector propagation.Injector) (Span, error) {
	...
	s, _, _, err := t.CreateLocalSpan(ctx, WithSpanType(SpanTypeExit), WithOperationName(operationName))
	if err != nil {
		return nil, err
	}
